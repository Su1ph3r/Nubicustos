...
from permission_requirements import (
    AWS_TOOLS, AWS_MANAGED_POLICIES, PROWLER_ADDITIONS_POLICY,
    AZURE_TOOLS, AZURE_BUILTIN_ROLES, AZURE_GRAPH_PERMISSIONS,
    GCP_TOOLS, GCP_PREDEFINED_ROLES, CLOUDSPLOIT_GCP_CUSTOM_ROLE,
    KUBERNETES_TOOLS, KUBESCAPE_CLUSTER_ROLE, TRIVY_CLUSTER_ROLE,
)
...
def load_aws_credentials(args) -> Optional[Dict]:
...
def load_azure_credentials(args) -> Optional[Dict]:
...
def load_gcp_credentials(args) -> Optional[Dict]:
...
def load_kubernetes_credentials(args) -> Optional[Dict]:
    ...
    try:
        ...
        for tool_key, tool_config in AWS_TOOLS.items():
            tool_result = check_aws_tool_permissions(
                iam, sts, session, tool_key, tool_config, principal_type, principal_name, arn
            )
        ...
        return ProviderResult(
            provider='aws',
            account_info='',
            connection_error='No AWS credentials found'
        )
    except ClientError as e:
        return ProviderResult(
            provider='aws',
            account_info='',
            connection_error=f'AWS API error: {e}'
        )
    except Exception as e:
        return ProviderResult(
            provider='aws',
            account_info='',
            connection_error=f'Error: {e}'
        )


def check_aws_tool_permissions(iam, sts, session, tool_key: str, tool_config: dict,
                                principal_type: str, principal_name: str, arn: str) -> PermissionResult:
    """Check permissions for a specific AWS tool."""
    from botocore.exceptions import ClientError

    result = PermissionResult(
        tool_name=tool_key,
        tool_display_name=tool_config['name'],
        passed=True
    )
    ...
    try:
        if principal_type == 'user':
            ...
            try:
                ...
                result.notes.append("Could not list role policies (may lack iam:ListAttachedRolePolicies)")
...
def check_azure_permissions(credentials: Dict, args) -> ProviderResult:
    ...
    try:
        ...
        if not principal_id:
            ...
            logger.warning("Using client_id for role lookup. For accurate results, provide --azure-object-id or add 'objectId' to credentials file")
        ...
        for tool_key, tool_config in AZURE_TOOLS.items():
            tool_result = check_azure_tool_permissions(
                tool_key, tool_config, assigned_roles
            )
...
def check_azure_tool_permissions(tool_key: str, tool_config: dict,
                                  assigned_roles: set) -> PermissionResult:
...
def check_gcp_permissions(credentials: Dict, args) -> ProviderResult:
    ...
    try:
        ...
        for tool_key, tool_config in GCP_TOOLS.items():
            tool_result = check_gcp_tool_permissions(
                tool_key, tool_config, assigned_roles
            )
            result.tool_results.append(tool_result)
...
def check_gcp_tool_permissions(tool_key: str, tool_config: dict,
                                assigned_roles: set) -> PermissionResult:
...
def check_kubernetes_permissions(credentials: Dict, args) -> ProviderResult:
    ...
    try:
        ...
        account_info = f"Context: {context_name} | Cluster: {cluster_name} | K8s: {version_info.git_version}"
        ...
        for tool_key, tool_config in KUBERNETES_TOOLS.items():
            tool_result = check_kubernetes_tool_permissions(
                auth_api, tool_key, tool_config
            )
            result.tool_results.append(tool_result)
...
def check_kubernetes_tool_permissions(auth_api, tool_key: str,
                                       tool_config: dict) -> PermissionResult:
                ...
                for verb in required_verbs:
                    ...
                    try:
                        ...
                        if not response.status.allowed:
                            perm_str = f"{verb} on {api_group}/{resource}" if api_group else f"{verb} on {resource}"
...
def print_results(results: List[ProviderResult], args):
    ...
    if total_fail == 0:
        ...
        print(f"{Colors.YELLOW}SUMMARY: {total_pass}/{total} tools ready | {total_fail} tools need attention{Colors.NC}")
    print("=" * 80)
...
def print_aws_remediation(tool_result: PermissionResult):
    ...
    for policy in tool_result.missing_policies:
        ...
        print(f"    --user-name YOUR_USER_NAME \\")
        print(f"    --policy-arn {policy_arn}")
        print()
        print("# For IAM Role:")
        print(f"aws iam attach-role-policy \\")
        print(f"    --role-name YOUR_ROLE_NAME \\")
        print(f"    --policy-arn {policy_arn}")
        print("```")
        print()
...
def print_azure_remediation(tool_result: PermissionResult):
    ...
    for role in tool_result.missing_roles:
        ...
        print(f'    --role "{role}" \\')
        print(f'    --scope /subscriptions/YOUR_SUBSCRIPTION_ID')
        print("```")
        print()
...
def print_gcp_remediation(tool_result: PermissionResult):
    ...
    for role in tool_result.missing_roles:
        if 'custom' in role.lower():
            ...
            print(f'3. Click "Add another role"')
...
def export_markdown(results: List[ProviderResult], failed_tools: List,
        ...
        for provider, tool_result in failed_tools:
            ...
            if provider == 'aws':
                for policy in tool_result.missing_policies:
                    ...
                    lines.append("```bash")
                    lines.append(f"aws iam attach-role-policy --role-name YOUR_ROLE --policy-arn {policy_arn}")
                    lines.append("```")
                    lines.append("")
                ...
                for role in tool_result.missing_roles:
                    ...
                    lines.append("```bash")
                    lines.append(f'az role assignment create --assignee YOUR_SP_ID --role "{role}" --scope /subscriptions/YOUR_SUB_ID')
                    lines.append("```")
                    lines.append("")
                ...
                for role in tool_result.missing_roles:
                    ...
                    lines.append("```bash")
                    lines.append(f'gcloud projects add-iam-policy-binding YOUR_PROJECT --member="serviceAccount:YOUR_SA" --role="{role}"')
                    lines.append("```")
                    lines.append("")
...
def export_text(results: List[ProviderResult], failed_tools: List,
                output_path: Path, args):
...
Examples:
  ...
  %(prog)s --provider kubernetes --kubeconfig ~/.kube/config
    ...
    azure_group.add_argument('--azure-object-id', help='Azure service principal object ID (for role lookups)')
    ...
    output_group.add_argument('--remediation', '-r', action='store_true',
...
def main():
    ...
    for provider in providers_to_check:
        if provider == 'aws':
            ...
            if creds:
                if args.dry_run:
                    print(f"[DRY-RUN] Would check AWS permissions (credentials found)")
                else:
            ...
            if creds:
                if args.dry_run:
                    print(f"[DRY-RUN] Would check Azure permissions (credentials found)")
                else:
            ...
            if creds:
                if args.dry_run:
                    print(f"[DRY-RUN] Would check GCP permissions (credentials found)")
                else:
            ...
            if creds:
                if args.dry_run:
                    print(f"[DRY-RUN] Would check Kubernetes permissions (credentials found)")
                else:
