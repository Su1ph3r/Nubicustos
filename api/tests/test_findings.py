...
class TestFindingsFilters:
    ...
    def test_filter_by_resource_type(
        ...
        data = response.json()
        assert all(
            f["resource_type"] == "EC2::Instance" for f in data["findings"]
        )
...
class TestGetFinding:
    ...
    def test_get_finding_includes_aggregated_data(
        self, client: TestClient, sample_finding
    ) -> None:
...
class TestFindingsByResource:
    ...
    def test_get_findings_by_resource(
        self, client: TestClient, sample_finding
    ) -> None:
        """Test getting findings for a specific resource."""
        response = client.get(
            f"/api/findings/by-resource/{sample_finding.resource_id}"
        )
    ...
    def test_get_findings_by_resource_not_found(
