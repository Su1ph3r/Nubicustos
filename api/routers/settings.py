...
from models.database import get_db, UserSetting
...
DEFAULT_SETTINGS = {
    "default_scan_profile": {"value": "quick", "category": "scans", "description": "Default scan profile to use"},
    "default_regions": {"value": ["us-east-1", "us-west-2"], "category": "scans", "description": "Default AWS regions to scan"},
    "default_severity_filter": {"value": ["critical", "high"], "category": "scans", "description": "Default severity levels to include"},
    "auto_cleanup_days": {"value": 90, "category": "data", "description": "Auto-delete scan results older than N days"},
    "export_format": {"value": "json", "category": "data", "description": "Default export format (json, csv)"},
    "max_concurrent_scans": {"value": 3, "category": "scans", "description": "Maximum concurrent scan executions"},
    "notifications_enabled": {"value": False, "category": "notifications", "description": "Enable notification system"},
    "webhook_url": {"value": None, "category": "notifications", "description": "Webhook URL for notifications"},
    "webhook_events": {"value": ["scan_complete", "critical_finding"], "category": "notifications", "description": "Events that trigger webhooks"},
    "email_alerts_enabled": {"value": False, "category": "notifications", "description": "Enable email alerts"},
    "email_alert_address": {"value": None, "category": "notifications", "description": "Email address for alerts"},
    "email_alert_threshold": {"value": "critical", "category": "notifications", "description": "Minimum severity for email alerts"},
    "theme": {"value": "dark", "category": "display", "description": "UI theme (dark, light)"},
    "findings_per_page": {"value": 50, "category": "display", "description": "Default items per page in findings list"},
}
...
async def list_settings(
    ...
    return UserSettingListResponse(
        settings=[UserSettingResponse.model_validate(s) for s in settings],
        total=len(settings)
    )
...
async def update_setting(
    ...
    if setting_key == "auto_cleanup_days":
        if not isinstance(update.value, int) or update.value < 1 or update.value > 365:
            raise HTTPException(
                status_code=400,
                detail="Auto cleanup days must be an integer between 1 and 365"
            )
    ...
    if setting_key == "max_concurrent_scans":
        if not isinstance(update.value, int) or update.value < 1 or update.value > 10:
            raise HTTPException(
                status_code=400,
                detail="Max concurrent scans must be an integer between 1 and 10"
            )
...
async def reset_settings(
    db: Session = Depends(get_db),
    category: Optional[str] = Query(None, description="Reset only settings in this category"),
):
    ...
    return SettingsResetResponse(
        message=f"Settings{category_msg} reset to defaults",
...
async def get_settings_by_category(
    ...
    ).order_by(UserSetting.setting_key).all()
