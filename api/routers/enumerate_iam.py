...
from models.database import get_db, EnumerateIamResult, ToolExecution
...
async def list_enumerate_iam_results(
    ...
    privesc_capable: Optional[bool] = Query(None, description="Filter by privesc capability"),
    admin_capable: Optional[bool] = Query(None, description="Filter by admin capability"),
    ...
    results = query.order_by(
        ...
        desc(EnumerateIamResult.created_at)
...
async def get_enumerate_iam_summary(db: Session = Depends(get_db)):
    ...
    privesc = db.query(EnumerateIamResult).filter(EnumerateIamResult.privesc_capable == True).count()
    admin = db.query(EnumerateIamResult).filter(EnumerateIamResult.admin_capable == True).count()
    data_access = db.query(EnumerateIamResult).filter(EnumerateIamResult.data_access_capable == True).count()
...
async def get_enumerate_iam_result(
    result_id: int,
    db: Session = Depends(get_db)
):
...
async def get_result_by_principal(
    principal_arn: str,
    db: Session = Depends(get_db)
):
    """Get enumerate-iam result for a specific principal ARN."""
    result = db.query(EnumerateIamResult).filter(
        EnumerateIamResult.principal_arn == principal_arn
    ).first()
...
async def get_result_permissions(
    result_id: int,
    db: Session = Depends(get_db)
):
...
async def run_enumerate_iam(
    request: EnumerateIamRunRequest,
    db: Session = Depends(get_db)
):
    """
    ...
    if not request.access_key or not request.secret_key:
        raise HTTPException(
            status_code=400,
            detail="access_key and secret_key are required for enumerate-iam"
        )
