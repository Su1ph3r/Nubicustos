...
from models.database import get_db, ImdsCheck
...
async def list_imds_checks(
    ...
    imds_v1_enabled: Optional[bool] = Query(None, description="Filter by IMDSv1 status"),
    ssrf_vulnerable: Optional[bool] = Query(None, description="Filter by SSRF vulnerability"),
...
async def get_imds_summary(db: Session = Depends(get_db)):
    ...
    container_exp = db.query(ImdsCheck).filter(
        ImdsCheck.container_credential_exposure == True
    ).count()

    region_counts = dict(
        db.query(ImdsCheck.region, func.count(ImdsCheck.id))
        .group_by(ImdsCheck.region)
        .all()
    )
...
async def get_imds_by_instance(
    instance_id: str,
    db: Session = Depends(get_db)
):
...
async def update_imds_remediation(
    ...
    return {"message": "Remediation status updated", "check_id": check_id, "new_status": status.lower()}
