...
func TestAllChecksUnique(t *testing.T) {
	...
	// + SQL(3) + Cosmos(2) + Defender(1) + RBAC(1) + Entra(3) + exposed-secret(1) = 23.
	...
	// Plus the policy-as-code rules umbrella check = 1. Total = 96.
	const wantChecks = 96
	...
	// cloudsql, compute, kms, gke = 8; K8s pods, rbac, secrets-scan = 3. Total = 42.
	const wantCollectors = 42
